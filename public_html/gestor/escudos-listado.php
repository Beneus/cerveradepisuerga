...
var urldest = "escudos-eliminar.php";
...
location.href = "escudos-listado.php?idsubsevicio="+x.value;

}
function SeleccionNucleoUrbano(x){
			location.href = "escudos-listado.php?Mostrar=<?php echo $Mostrar;?>&idNucleoUrbano="+x.value;
			
		}
function CambiarClasificacion(x){
location.href = "escudos-listado.php?Mostrar=<?php echo $Mostrar;?>&TipoTramo="+x.value;

}
function Paginar(x){
location.href = "escudos-listado.php?idNucleoUrbano=<?php echo $idNucleoUrbano;?>&Mostrar="+x.value;

}
function CambiarPagina(x){
location.href = "escudos-listado.php?Mostrar=<?php echo $Mostrar;?>&idNucleoUrbano=<?php echo $idNucleoUrbano;?>&Pagina="+x.value;
...
<ul>
	...
	<li class="liselect"><a title="Listado del directorio"  href="escudos-listado.php">Listado de escudos</a></li>
</ul>
</div>
...
$sql = "SELECT ESC.* FROM Escudos as ESC "
." inner join NucleosUrbanos as NU ON ESC.idNucleoUrbano = NU.idNucleoUrbano "
." where idEscudo > 0 ";
if ($idNucleoUrbano > 0){ $sql = $sql . " and ESC.idNucleoUrbano = $idNucleoUrbano ";}
if($Buscar != ""){ $sql .= " and (Nombre like '%$Buscar%' or Direccion like '%$Buscar%' or ESC.Descripcion like '%$Buscar%') ";}
...
if($row["ImgDescripcion"] > 0){
   		...
   		$resultImg = mysqli_query($link,$sqlImg);
			...
			if($maxImg > 0){
				...
				echo "<img src=\"".str_replace("images","thumb","../".$Path."/".$Archivo)."\" width=\"$AnchoThumb\" height=\"$AltoThumb\" title=\"$Titulo\" alt=\"$Titulo\" />";
			}
...
<td width="100"><div align="center">
<input type="checkbox" name="idDir" value="<?php echo $row["idEscudo"];?>"/>
</div></td>
<td width="100"><div align="center">
  <input type="button" name="EDITAR" value="Editar" class="boton_button" onclick="location.href='escudos-editar.php?idEscudo=<?php echo $row["idEscudo"];?>&page=<?php echo $pagina_actual;?>&titulo=<?php echo $titulo;?>'"/>
</div></td>
<td class="lateralizq">&nbsp;</td>
</tr>
...
<td>&nbsp;</td>
<td>&nbsp;</td>
<td width="100"><div align="center"><input type="button" name="ELIMINAR" value="Eliminar" class="boton_button" onClick="EliminarEntrada(<?php echo $pagina_actual;?>)"/></div></td>
<td width="100"><div align="center"></div></td>
<td class="lateralizq">&nbsp;</td>
</tr>
<tr class="titulolistado">
...
echo "<a href=\"escudos-listado.php?Pagina=".($Pagina - 1)."&Mostrar=$Mostrar&buscar=$Buscar&Clasificacion=$Clasificacion&Clase=$Clase\" class=\"linkBlanco\"> << Anterior </a>";
}
if(($Pagina < $numPags ) and ($Mostrar < $NumTotalRegistros)){
echo "<a href=\"escudos-listado.php?Pagina=".($Pagina + 1)."&Mostrar=$Mostrar&buscar=$Buscar&Clasificacion=$Clasificacion&Clase=$Clase\" class=\"linkBlanco\"> Siguiente >> </a>";
}
?></td>
<td>
<?php
...
?>
<div class="errortexto">No hay entradas.</div>
<?php
}
